...
var Player = new Class({
  ...
  load: function(file_path , chain , pause){
    ...
    child.destroy = function(signal){
      process.killRecursive(child.pid, signal);
    }
    ...
    child.on('close', function(code , signal){
      ...
      if(child.inBack && self.playedOnce){
        console.log('end prematurely')

        delay = self.front.duration - self.back.duration ;
